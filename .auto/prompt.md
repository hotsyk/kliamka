...
- **Kept — argv-aware prog memoization:** small general win; dynamic `sys.argv[0]` remains keyed correctly.
...
- **Kept — lazy standard help + bulk Action attachment:** standard help remains publicly equivalent but materializes its independent indexed Action only when usage/help is rendered; model Actions clone/attach in positional/optional batches. This pushed the aggregate well below argparse.
- **Kept — module CLI metadata helper + direct extra-ignore validation:** avoids classmethod binding and returns directly through Pydantic's compiled validator on the common path.
- **Current best:** `kliamka_geomean_us=12.9527`, creation 5.38us, parsing 10.36us, full workflow 18.28us, validation 1.04us, repeated-100 916.04us, matched argparse ratio 0.334. This is 72.9% below the 47.7634 baseline and about 3x faster than argparse in the matched geometric aggregate.
- **Later exploration under machine drift:** full parser-graph prototypes, settings/validator caching, completeness specializations, SUPPRESS/deferred defaults, and many cloning/dict-layout variants failed strict primary acceptance. Do not revisit exhausted paths in `.auto/log.jsonl` runs 28–74.
- Packaging smoke tests and the Python 3.11–3.14 Docker matrix pass. Persistent cache/help/extension assertions run from `.auto/checks.sh` after every benchmark.
